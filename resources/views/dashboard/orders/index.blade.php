...
<div class="card">
    <div class="card-header d-flex justify-content-between align-items-center">
        <h3 class="card-title">تفال جميع الطلبات</h3>
            ...
            <tbody>
                ...
                <tr>
                        ...
                        @if ($order->payment_method == 'zaincash')
                            @if ($order->payment_image)
                                <a href="{{ asset('storage/' . $order->payment_image) }}" target="_blank">
                                    <img src="{{ asset('storage/' . $order->payment_image) }}" alt="صور الدفع" width="80">
                                </a>
                            @else
                                <span class="text-danger">لا توجد صرة</span>
                            @endif
                        @else
                    ...
                    <td>
                        ...
                        مبنى: {{ $order->address->building ?? 'لصيدلية ' }}
                    </td>
  <td>{{ $order->notes }}</td>

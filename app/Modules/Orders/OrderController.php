...
//                 $giftDescription = "هدية عد راء {$rule->min_quantity} ن العروض لحددة  هدية: " .
...
//                 $giftDescription = "هدية عند الشراء قية {$rule->min_amount} من اعرو المحددة → ه: " .
//                     ucfirst($giftTarget->target_type) . " ID: {$giftTarget->target_id}";
//                 $giftRuleId = $rule->id;
//                 break;
//             }
...
public function checkout(Request $request)
{
    ...
    if (!empty($stockErrors)) {
        return response()->json([
            'message' => 'بعض المنتجات غير متوفرة بالكميات المطلوبة',
            'errors' => $stockErrors,
        ], 422);
    }
...
public function index()
{
 ...
 public function track($id)
    ...
    {
        return match ($status) {
            'pending' => 'قيد المرجعة من إدر',
            'confirmed' => 'تم تكيد الطلب وار الحضير',
            'canceled' => 'تم إل الطلب',
            'completed' => 'م وصيل ال بنجاح',
            default => 'غير معوف'
        };
    }

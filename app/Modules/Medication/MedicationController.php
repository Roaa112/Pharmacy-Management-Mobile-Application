...
//             // specific_days -> نظهر فقط إذا اليوم موجد في الأيام المحددة
//             if ($medication->repeat_type === 'specific_days') {
...
//             // عشان نرجع كل اعيد الدواء مع حالة taken/pending بناءً على التاريخ واليوم
//             $medication->times = $medication->times->map(function ($time) use ($medication, $user, $date) {
...
//     // جب كل الأدوية مع اعلاقات الضرورية فعة واحة
...
//     foreach ($daysOfWeek as $day) {
//         // حساب تاري اليوم في الأبوع الحالي أو التالي
//         $date = Carbon::now()->startOfWeek(); // بدية الأسبوع (الإثنين)
//         $dayIndex = array_search($day, $daysOfWeek);

//         // إذا يوم البت هو البداية حسب منطقتك، تأكد من ضب startOfWeek
//         // ما لو أسبوعك يبدأ لسبت:
//         // $date = Carbon::now()->startOfWeek(Carbon::SATURDAY);

//         // إضافة عدد الأيا للصول لليوم المطلو
//         $date = $date->copy()->addDays($dayIndex);

//         // إذا التاريخ أغر من اليوم (أي في الماضي)، عد التاريخ لليوم التالي نسه
//         if ($date->lt(Carbon::today())) {
//             $date = Carbon::today();
//         }

//         // فلتر الأدوي حسب نوع اتكرار اليوم
//         $filteredMeds = $allMedications->filter(function ($medication) use ($day, $date, $user) {
...
//             if ($medication->repeat_type === 'specific_days') {
//                 // نفترض علاقة days حوي كائنات أو أساء الأيام كخاصية 'day'
//                 return $medication->days->contains(function ($d) use ($day) {
//                     if (is_object($d)) {
//                         return Str::lower($d->day) === $day;
//                     }
...
//             // تعديل كل الأوات ليشمل حاة الدواء في هذا اوقت والتاريخ
//             $medication->times = $medication->times->map(function ($time) use ($medication, $user, $date) {
...
//                 // إضافة اخاصة بدون تحويل الكائن إلى مصفوفة
//                 $time->status = $log ? $log->status : 'pending';
 ...
 public function updateLogStatus(Request $request, Medication $medication)
{
    ...
    if ($shownDate->gt(Carbon::today())) {
        return response()->json([
            'success' => false,
            'message' => 'لا يمكن تعديل حالة جرعة في المستقبل',
        ], 422);
    }
    ...
    return response()->json([
        'success' => true,
        'message' => 'تم تحديث حالة الجرعة بنجاح',
    ]);
}
